...
from mapie.estimator.quantile_regressor import QuantileEnsembleRegressor
...
STRATEGIES = {
    ...
    "cqr_cv+": {"method": "plus", "cv": KFold(n_splits = 20, shuffle=True), "alpha": 0.1}
}
y_pred, y_pis = {}, {}
...
for strategy, params in STRATEGIES.items():
    ...
    elif strategy == "cqr_cv+": 
        mapie = QuantileEnsembleRegressor(estimator, **params)
...
def get_coverages_widths_by_bins(
    ...
    """
    cuts = []
    cuts_ = pd.qcut(y_test["cqr"], bins).unique()[:-1]
    for item in cuts_:
        cuts.append(item.left)
    cuts.append(cuts_[-1].right)
    cuts.append(np.max(y_test["cqr"])+1)
    recap = {}
